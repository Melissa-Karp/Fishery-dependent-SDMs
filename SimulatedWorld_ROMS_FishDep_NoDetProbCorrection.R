...
closed_area<-define_closed_area(closed_area=c(-124,-116,31,35))
#plot(closed_area)
# par(mfrow=c(1,1))
...
SimulateWorld_ROMS_FishDepFun_NODetProb <- function(dir, nsamples){
  ...
  output <- as.data.frame(matrix(NA, nrow=21912*121,ncol=16)) #21912 non-NA grid cells in ROMS
  colnames(output) <- c("lon","lat","year","pres","suitability_t","suitability_t1","random_sampled","pref_sampled", "Dist_sampled", "BY_sampled", "ByDist_sampled", "Closed_sampled", "sst","zoo_200", "mld", "chl_surface")
  ...
  for (y in 1:121){
    ...
    #Dist_params <- formatFunctions(min_dist = c(fun="logisticFun",alpha =150,beta =700),
    #                               spB_t1 = c(fun="logisticFun",alpha=-0.05,beta=0.5))   
    ...
    ##### 5. Static Closed Area ######
    Tar_closed<-Tar_reclassify*closed_area
    plot(Tar_closed)
    ...
    #****** 5. Closed Area Sampling - based on HABITAT SUITABILITY of target species & Static Closed Area**********
    presence.points.close<-sampleOccurrences(suitability_PA, n=nsamples, type="presence-absence",
                                            detection.probability = 1, bias = "manual", weights = Tar_closed, plot = TRUE)
    #convert to dataframe
    ...
    #add to dataframe
    df_full_7 <- left_join(df_full_6, close_df_pref, by=c('x','y'))
    df_full_7$Closed_sampled <- ifelse(is.na(df_full_7$Closed_sampled),0,df_full_7$Closed_sampled)
    
    colnames(df_full_7)[1:2] <- c("lon","lat")
    ...
    temp_random <-   left_join(output[se:ei,1:6], df_full_7[,c(1,2,4)], by=c('lon','lat')) ##########SB: changed these column indices###########
    temp_pref <-   left_join(output[se:ei,1:6], df_full_7[,c(1,2,6)], by=c('lon','lat'))##########SB: changed these column indices###########
    temp_Dist <-   left_join(output[se:ei,1:6], df_full_7[,c(1,2,8)], by=c('lon','lat'))##########SB: changed these column indices###########
    temp_BY<- left_join(output[se:ei,1:6], df_full_7[,c(1,2,10)], by=c('lon','lat'))##########SB: changed these column indices###########
    temp_ByDist<- left_join(output[se:ei,1:6], df_full_7[,c(1,2,12)], by=c('lon','lat'))##########SB: changed these column indices###########
    temp_Closed<- left_join(output[se:ei,1:6], df_full_7[,c(1,2,14)], by=c('lon','lat'))##########SB: changed these column indices###########
        #assign to output
