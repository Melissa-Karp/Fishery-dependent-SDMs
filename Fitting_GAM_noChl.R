...
if("tar_0.7" %in% sampling) {
  ...
  abundxT_3_nochl <- predict(gam_Tar_N_3, dat_fcast, type="response")
...
if("tar_0.8" %in% sampling) {
  ...
  dat_hist$presxT_4_nochl <- predict(gam_Tar_P_4, dat_hist, type="response")
  abundxT_4_nochl <- predict(gam_Tar_N_4, dat_hist, type="response")
  dat_hist$gam_Tar_0.8_nochl <- dat_hist$presxT_4_nochl * exp(abundxT_4_nochl)
  
  dat_fcast$presxT_4_nochl <- predict(gam_Tar_P_4, dat_fcast, type="response")
  abundxT_4_nochl <- predict(gam_Tar_N_4, dat_fcast, type="response")
...
if("tar_0.9" %in% sampling) {
  ...
  dat_fcast$presxT_5_nochl <- predict(gam_Tar_P_5, dat_fcast, type="response")
...
if("npo" %in% sampling){
  ...
  dat_fcast$presxD_npo_nochl <- predict(gam_Dist_P_npo, dat_fcast, type="response")
  abundxD_npo_nochl <- predict(gam_Dist_N_npo, dat_fcast, type="response")
...
if("npn" %in% sampling){
  ...
  dat_fcast$presxD_npn_nochl <- predict(gam_Dist_P_npn, dat_fcast, type="response")
...
if("mpn" %in% sampling){
  ...
  dat_fcast$presxD_mpn_nochl <- predict(gam_Dist_P_mpn, dat_fcast, type="response")
...
if("CA_lar" %in% sampling) {
  ...
  dat_hist$gam_CA_lar_nochl <- dat_hist$presxCA_lar * exp(abundxCA_lar)
