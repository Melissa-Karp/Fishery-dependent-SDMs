...
SimulateWorld_ROMS_Albacore_SB <- function(dir){
 ...
 #----SPECIES B (albacore): assign response curves----
    ...
    #plot(spB_suitability$suitab.raster) #plot habitat suitability
    #save suitability raster to local drive
    writeRaster(spB_suitability$suitab.raster, "~/DisMAP project/Location, Location, Location/Location Workshop/Albacore_suitability_rasters/Alb_suit_[y].grd")
...
#-----Sample Presences and Absences:: Adding in SAMPLING BIASES based on FISHER BEHAVIOR-----
     #1. calculate distance of each cell from Ports 1-5 - this is done in DistancetoPort.R file to create dataframe with distance of each cell to each port location
    
    # dist_all_ports <- Dist_to_Ports[,which(colnames(Dist_to_Ports)=="dp1"):which(colnames(Dist_to_Ports)=="dp5")]
    # dist_Pxx <- dist_all_ports[,port_start]    #distances to all cells from this departure port 
    # 
     #2. for y-1, calculate some fisher expectation of catch for every cell (based on habitat suitability/abundance raster & some random noise)
    
     #3. for y, calculate utility for every cell (using Utility = ExpectedCatch * Price - Distance * Cost) seperately for each port
     
     #4. calculate the maximum utility for each cell (max of the 5 port values)
...
#Test
test <- SimulateWorld_ROMS_Albacore_SB(dir = dir)
