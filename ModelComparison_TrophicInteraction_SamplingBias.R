...
#Run this function
dat <- SimulateWorld_ROMS_TrophicInteraction_SB(dir=dir) #takes a few mins
...
#Run if lognormal response was simulated
brt1.a <- gbm.fixed(data=dat_hist[dat_hist$abundance>0,], gbm.x = c("temp", "chla"),gbm.y = 'log_abundance',family = "gaussian",tree.complexity = 3, learning.rate = 0.01, n.trees=1000, bag.fraction = 0.6)
brt1.p <- gbm.fixed(data=dat_hist, gbm.x = c("temp", "chla"),gbm.y = 'pres',family = "bernoulli",tree.complexity = 3, learning.rate = 0.01, n.trees=1000, bag.fraction = 0.6)
