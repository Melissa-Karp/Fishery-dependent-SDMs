...
if("ran" %in% sampling){
  ...
  gam_Ran_P_nochlte <- gam(pres~s(temp) + s(mld) + te(lat,lon,year), data=dat_hist_random, family=binomial) 
  #plot(gam_Ran_P_nochlte, pages=1)
  gam_Ran_N_nochlte <- gam(log_abundance~s(temp)+ s(mld) + te(lat,lon,year), data=dat_hist_random[dat_hist_random$abundance>0,], family=gaussian)
...
if("tar_0.5" %in% sampling) {
  ...
  gam_Tar_P_1_nochlte <- gam(pres~s(temp)+ s(mld) + te(lat,lon,year), data=dat_hist_Tar_1, family=binomial)
  #plot(gam_Tar_P_1_nochlte, pages=1)
  gam_Tar_N_1_nochlte <- gam(log_abundance~s(temp)+ s(mld) + te(lat,lon,year), data=dat_hist_Tar_1[dat_hist_Tar_1$abundance>0,], family=gaussian)
...
if("tar_0.6" %in% sampling) {
  ...
  gam_Tar_P_2_nochlte <- gam(pres~s(temp)+ s(mld) + te(lat,lon,year), data=dat_hist_Tar_2, family=binomial)
  #plot(gam_Tar_P_2_nochlte, pages=1)
  gam_Tar_N_2_nochlte <- gam(log_abundance~s(temp)+ s(mld) + te(lat,lon,year), data=dat_hist_Tar_2[dat_hist_Tar_2$abundance>0,], family=gaussian)
...
if("tar_0.7" %in% sampling) {
  ...
  gam_Tar_P_3_nochlte <- gam(pres~s(temp)+ s(mld) + te(lat,lon,year), data=dat_hist_Tar_3, family=binomial)
  #plot(gam_Tar_P_3_nochlte, pages=1)
  gam_Tar_N_3_nochlte <- gam(log_abundance~s(temp)+ s(mld) + te(lat,lon,year), data=dat_hist_Tar_3[dat_hist_Tar_3$abundance>0,], family=gaussian)
...
if("tar_0.8" %in% sampling) {
  ...
  gam_Tar_P_4_nochlte <- gam(pres~s(temp)+ s(mld) + te(lat,lon,year), data=dat_hist_Tar_4, family=binomial)
  #plot(gam_Tar_P_4_nochlte, pages=1)
  gam_Tar_N_4_nochlte <- gam(log_abundance~s(temp)+ s(mld) + te(lat,lon,year), data=dat_hist_Tar_4[dat_hist_Tar_4$abundance>0,], family=gaussian)
...
if("tar_0.9" %in% sampling) {
  ...
  gam_Tar_P_5_nochlte <- gam(pres~s(temp)+ s(mld) + te(lat,lon,year), data=dat_hist_Tar_5, family=binomial)
  #plot(gam_Tar_P_5_nochlte, pages=1)
  gam_Tar_N_5_nochlte <- gam(log_abundance~s(temp)+ s(mld) + te(lat,lon,year), data=dat_hist_Tar_5[dat_hist_Tar_5$abundance>0,], family=gaussian)
...
if("npo" %in% sampling){
  ...
  gam_Dist_P_npo_nochlte <- gam(pres~s(temp) + s(mld) + te(lat,lon,year), data=dat_hist_Dist_npo, family=binomial)
  #plot(gam_Dist_P_npo_nochlte, pages=1)
  gam_Dist_N_npo_nochlte <- gam(log_abundance~s(temp) + s(mld) + te(lat,lon,year), data=dat_hist_Dist_npo[dat_hist_Dist_npo$abundance>0,], family=gaussian)
...
if("npn" %in% sampling){
  ...
  gam_Dist_P_npn_nochlte <- gam(pres~s(temp) + s(mld) + te(lat, lon, year), data=dat_hist_Dist_npn, family=binomial)
  #plot(gam_Dist_P_npn_nochlte, pages=1)
  gam_Dist_N_npn_nochlte <- gam(log_abundance~s(temp) + s(mld) + te(lat, lon, year), data=dat_hist_Dist_npn[dat_hist_Dist_npn$abundance>0,], family=gaussian)
...
if("mpo" %in% sampling){
  ...
  gam_Dist_P_mpo_nochlte <- gam(pres~s(temp) + s(mld) + te(lat,lon,year), data=dat_hist_Dist_mpo, family=binomial)
  #plot(gam_Dist_P_mpo_nochlte, pages=1)
  gam_Dist_N_mpo_nochlte <- gam(log_abundance~s(temp) + s(mld) + te(lat,lon,year), data=dat_hist_Dist_mpo[dat_hist_Dist_mpo$abundance>0,], family=gaussian)
...
if("mpn" %in% sampling){
  print("Fitting GAM-NPN_nochl_te")
  gam_Dist_P_mpn_nochlte <- gam(pres~s(temp) + s(mld) + te(lat,lon,year), data=dat_hist_Dist_mpn, family=binomial)
  #plot(gam_Dist_P_mpn_nochlte, pages=1)
  gam_Dist_N_mpn_nochlte <- gam(log_abundance~s(temp) + s(mld) + te(lat,lon,year), data=dat_hist_Dist_mpn[dat_hist_Dist_mpn$abundance>0,], family=gaussian)
...
if("spo" %in% sampling){
  ...
  gam_Dist_P_spo_nochlte <- gam(pres~s(temp) + s(mld) + te(lat,lon,year), data=dat_hist_Dist_spo, family=binomial)
  #plot(gam_Dist_P_spo_nochlte, pages=1)
  gam_Dist_N_spo_nochlte <- gam(log_abundance~s(temp) + s(mld) + te(lat,lon,year), data=dat_hist_Dist_spo[dat_hist_Dist_spo$abundance>0,], family=gaussian)
...
if("spn" %in% sampling){
  ...
  gam_Dist_P_spn_nochlte <- gam(pres~s(temp) + s(mld) + te(lat,lon,year), data=dat_hist_Dist_spn, family=binomial)
  #plot(gam_Dist_P_spn_nochlte, pages=1)
  gam_Dist_N_spn_nochlte <- gam(log_abundance~s(temp) + s(mld) + te(lat,lon,year), data=dat_hist_Dist_spn[dat_hist_Dist_spn$abundance>0,], family=gaussian)
...
if("allo" %in% sampling){
  ...
  gam_Dist_P_allo_nochlte <- gam(pres~s(temp) + s(mld) + te(lat,lon,year), data=dat_hist_Dist_allo, family=binomial)
  #plot(gam_Dist_P_allo_nochlte, pages=1)
  gam_Dist_N_allo_nochlte <- gam(log_abundance~s(temp) + s(mld) + te(lat,lon,year), data=dat_hist_Dist_allo[dat_hist_Dist_allo$abundance>0,], family=gaussian)
...
if("alln" %in% sampling){
  ...
  gam_Dist_P_alln_nochlte <- gam(pres~s(temp) + s(mld) + te(lat,lon,year), data=dat_hist_Dist_alln, family=binomial)
  #plot(gam_Dist_P_alln_nochte, pages=1)
  gam_Dist_N_alln_nochlte <- gam(log_abundance~s(temp) + s(mld) + te(lat,lon,year), data=dat_hist_Dist_alln[dat_hist_Dist_alln$abundance>0,], family=gaussian)
...
if("BY" %in% sampling){
  ...
  gam_BY_P_nochlte <- gam(pres~s(temp) + s(mld)+ te(lat,lon,year), data=dat_hist_BY, family=binomial)
  #plot(gam_BY_P_nochlte, pages=1)
  gam_BY_N_nochlte<- gam(log_abundance~s(temp) + s(mld)+ te(lat,lon,year), data=dat_hist_BY[dat_hist_BY$abundance>0,], family=gaussian)
...
if("CA_sm" %in% sampling) {
  ...
  gam_CA_P_sm_nochlte <- gam(pres~s(temp) + s(mld) + te(lat,lon,year), data=dat_hist_CA_sm, family=binomial)
  #plot(gam_CA_P_sm_nochlte, pages=1)
  gam_CA_N_sm_nochlte <- gam(log_abundance~s(temp) + s(mld) + te(lat,lon,year), data=dat_hist_CA_sm[dat_hist_CA_sm$abundance>0,], family=gaussian)
...
if("CA_med" %in% sampling) {
  ...
  gam_CA_P_med_nochlte <- gam(pres~s(temp) + s(mld) + te(lat,lon, year), data=dat_hist_CA_med, family=binomial)
  #plot(gam_CA_P_med_nochlte, pages=1)
  gam_CA_N_med_nochlte <- gam(log_abundance~s(temp) + s(mld) + te(lat,lon, year), data=dat_hist_CA_med[dat_hist_CA_med$abundance>0,], family=gaussian)
...
if("CA_lar" %in% sampling) {
  ...
  gam_CA_P_lar_nochlte <- gam(pres~ s(temp) + s(mld) + te(lat, lon, year), data=dat_hist_CA_lar, family=binomial)
  #plot(gam_CA_P_lar_nochlte, pages=1)
  gam_CA_N_lar_nochlte <- gam(log_abundance~ s(temp) + s(mld) +  te(lat, lon, year), data=dat_hist_CA_lar[dat_hist_CA_lar$abundance>0,], family=gaussian)
