...
if("mpn" %in% sampling){
  ...
  abundDx_mpn <- exp(predict(brt_dist_N_mpn, dat_hist, n.trees=brt_dist_N_mpn_nochl$gbm.call$best.trees, type="response"))
...
if("BY" %in% sampling){
  ...
  abundBx <- exp(predict(brt_B_N, dat_fcast, n.trees=brt_B_N_nochl$gbm.call$best.trees, type="response"))
